...
#define DEF_FAM_NEWDEL(T, FamMem)                                                                                                \
    static constexpr size_t Sizeof(size_t count) noexcept {                                                                      \
        return std::max<size_t>(sizeof(T), decltype(FamMem)::Sizeof(count, offsetof(T, FamMem)));                                \
    }                                                                                                                            \
                                                                                                                                 \
...
constexpr std::enable_if_t<!std::is_array<T>::value> destroy_at(T* ptr) noexcept(std::is_nothrow_destructible<T>::value) {
    ptr->~T();
}
DIAGNOSTIC_POP
template<typename T>
...
constexpr std::enable_if_t<std::is_integral<N>::value, T> destroy_n(T first, N count) {
    if (count != 0) {
        do {
            al::destroy_at(std::addressof(*first));
            ++first;
...
template<typename T, typename N>
inline std::enable_if_t<std::is_integral<N>::value, T> uninitialized_default_construct_n(T first, N count) {
...
struct FlexArrayStorage<T, alignment, true> {
    ...
    static constexpr size_t Sizeof(size_t count, size_t base = 0u) noexcept {
        return std::max<size_t>(offsetof(FlexArrayStorage, mArray) + sizeof(T) * count, sizeof(FlexArrayStorage)) + base;
    }

    FlexArrayStorage(size_t size) : mSize{size} { al::uninitialized_default_construct_n(mArray, mSize); }
